...
class FakeMirrorJiraClient:
    ...
    def __init__(self, existing_issuelinks: list[dict] | None = None, created_key: str = "JJST-8"):
...
def _issue_created_body(
    ...
    return {
        ...
        "issue": {
            ...
            "fields": {
                "project": {"key": project_key, "id": "11742"},
...
def test_issue_created_webhook_creates_mirror_and_link():
    ...
    client = FakeMirrorJiraClient(existing_issuelinks=[], created_key="JJST-8")
    ...
    assert result["source_issue"] == "JTT-111"
    assert result["mirror_issue"] == "JJST-8"
...
def test_issue_created_webhook_idempotency_guard_skips_already_linked_issue():
    ...
    signature = compute_x_hub_signature(SECRET, body)
    headers = {"X-Hub-Signature": signature}
    already_linked = [{"type": {"id": "10012"}, "outwardIssue": {"key": "JJST-8"}}]
...
def test_issue_created_webhook_ignores_unrelated_issue_links():
    ...
    client = FakeMirrorJiraClient(existing_issuelinks=unrelated_link, created_key="JJST-8")
...
def test_issue_created_webhook_returns_400_on_malformed_payload():
    ...
    malformed = {"webhookEvent": "jira:issue_created", "issue": {"key": "JTT-1", "fields": {}}}
...
def test_issue_created_service_request_vs_other_types_dont_crosscontaminate():
    """Regression guard: two issue_created events in sequence for
    different issuetypes must each resolve their own epic correctly,
    not leak state between calls (create_mirror/resolve_epic are pure
    functions, but worth locking in at the handler level too)."""
    from handler import handle_webhook

    body_sr = json.dumps(_issue_created_body(issue_key="JTT-1", issuetype_name="Service Request"))
    sig_sr = compute_x_hub_signature(SECRET, body_sr)
    client_sr = FakeMirrorJiraClient(created_key="JJST-10")
    handle_webhook(
        body_sr,
    ...
    body_ir = json.dumps(_issue_created_body(issue_key="JTT-2", issuetype_name="Incident Request"))
    sig_ir = compute_x_hub_signature(SECRET, body_ir)
    client_ir = FakeMirrorJiraClient(created_key="JJST-11")
    handle_webhook(
        body_ir,
    ...
    # Current TEST MIRROR_MAP has no epic split for JTT->JJST (parent_key
    # is None either way) - this test locks in that both calls succeeded
    # independently rather than one call's state leaking into the other.
    assert client_sr.create_issue_calls[0]["parent_key"] is None
    assert client_ir.create_issue_calls[0]["parent_key"] is None
