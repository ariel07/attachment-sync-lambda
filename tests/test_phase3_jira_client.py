...
class FakeSession:
    ...
    def get(self, url, auth=None, params=None, timeout=None, **kwargs):
        self.calls.append({"method": "GET", "url": url, "auth": auth, "params": params, "timeout": timeout})
        return self._responses.pop(0)

    def post(self, url, auth=None, headers=None, files=None, timeout=None, **kwargs):
        self.calls.append({
            "method": "POST", "url": url, "auth": auth,
...
def test_get_issue_raises_on_http_error():
    ...
    client = JiraClient("https://icxeed.atlassian.net", "svc@icxeed.ai", "fake-token", session=session)
...
def test_upload_attachment_sends_correct_multipart_request():
    ...
    assert call["files"] == {"file": ("image-20260812-021129.png", b"fake image bytes", "image/png")}
